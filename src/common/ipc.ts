...
export function broadcastIpc({ channel, webContentId, filter, args = [] }: IpcMessageOpts): void {
...
export function handleIpc(channel: IpcChannel, handler: IpcMessageHandler, options: IpcHandleOpts = {}): void {
...
// todo: improve api
export function createIpcChannel({ channel, autoBind, ...initOpts }: IpcPairOptions): { channel: string; handleInMain: (opts?: { handler?: IpcMessageHandler<any>; options?: IpcHandleOpts; }) => void; invokeFromRenderer(...args: any[]): Promise<any>; } {
  ...
  return {
    ...
    invokeFromRenderer(...args: any[]): Promise<any> {
