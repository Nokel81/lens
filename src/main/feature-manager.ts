...
export async function installFeature(name: string, cluster: Cluster, _config: Record<string, any>): Promise<void> {
  // TODO Figure out how to handle config stuff
  return ALL_FEATURES.get(name).install(cluster)
}

export async function upgradeFeature(name: string, cluster: Cluster, _config: Record<string, any>): Promise<void> {
  // TODO Figure out how to handle config stuff
