...
export class HelmChartDetails extends Component<Props> {
  ...
  private chartPromise: CancellablePromise<{ readme: string; versions: HelmChart[] }>;
      ...
      <div className="introduction flex align-flex-start">
        ...
        <div className="intro-contents box grow">
          ...
          <DrawerItem name={_i18n._(t`Home`)}>
            <a href={selectedChart.getHome()} rel="noreferrer" target="_blank">{selectedChart.getHome()}</a>
          </DrawerItem>
          <DrawerItem name={_i18n._(t`Maintainers`)} className="maintainers">
            {selectedChart.getMaintainers().map(({ name, email, url }) =>
              <a key={name} href={url ? url : `mailto:${email}`} rel="noreferrer" target="_blank">{name}</a>,
            )}
          </DrawerItem>
