...
export class PodDetailsList extends React.Component<Props> {
  ...
  renderCpuUsage(id: string, usage: number): React.ReactNode {
  ...
  renderMemoryUsage(id: string, usage: number): React.ReactNode {
  ...
  @autobind()
  renderTableRow(uid: string): React.ReactElement {
    ...
    return (
      <div className="PodDetailsList flex column">
        ...
        <Table
          ...
          getTableRow={this.renderTableRow}
          className="box grow"
        >
          <TableHead>
          ...
          {
            !virtual && pods.map(pod => this.renderTableRow(pod.getId()))
          }
        </Table>
      </div>
    )
