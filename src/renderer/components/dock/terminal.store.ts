...
export interface TerminalTab extends DockTabEntry {
  node?: string; // activate node shell mode
}

export function isTerminalTab(tab: DockTabEntry): boolean {
  return tab && tab.kind === TabKind.TERMINAL
}

export function createTerminalTab(tabParams: Partial<TerminalTab> = {}): DockTabEntry {
...
export class TerminalStore {
  ...
  async connect(tabId: TabId): Promise<void> {
    ...
    const tab: TerminalTab = dockStore.getTabById(tabId)
  ...
  disconnect(tabId: TabId): Promise<void> {
    if (!this.isConnected(tabId)) {
      return
    }
