...
export class DockStore {
  protected initialTabs: DockTabEntry[] = [
    { id: "terminal", kind: TabKind.TERMINAL, title: "Terminal" },
  ];

  protected storage = new StorageHelper("dock", {}); // keep settings in localStorage
  ...
  @observable tabs = observable.array<DockTabEntry>(this.initialTabs);
  @observable selectedTabId = this.defaultTabId;

  @computed get selectedTab(): DockTabEntry {
  ...
  protected checkMaxHeight(): boolean {
    if (!this.height) {
  ...
  onResize(callback: () => void, options?: IReactionOptions): () => void {
    return reaction(() => [this.height, this.fullSize], callback, options)
  }

  onTabChange(callback: (tabId: TabId) => void, options?: IReactionOptions): () => void {
  ...
  getTabById(tabId: TabId): DockTabEntry {
  ...
  @action
  createTab(anonTab: DockTabEntry, addNumber = true): DockTabEntry {
    const tabId = MD5(Math.random().toString() + Date.now()).toString()
    const tab: DockTabEntry = { id: tabId, ...anonTab }
    if (addNumber) {
