...
import { KubeWatchEvent, kubeWatchApi } from "./api/kube-watch-api"
...
export abstract class KubeObjectStore<T extends KubeObject = any> extends ItemStore<T> {
  ...
  protected eventsBuffer = observable<KubeWatchEvent<KubeJsonApiData>>([], { deep: false });

  protected bindWatchEventsUpdater(delay = 1000): IReactionDisposer {
    return reaction(() => this.eventsBuffer.toJS()[0], this.updateFromEventsBuffer, { delay })
  }
  ...
  protected onWatchApiEvent(evt: KubeWatchEvent): void {
