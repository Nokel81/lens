...
import { cancelableFetch, CancellablePromise } from "../utils/cancelableFetch"
...
export class JsonApi<D = JsonApiData, P extends JsonApiParams = JsonApiParams> {
  ...
  get<T = D>(path: string, params?: P, reqInit: RequestInit = {}): CancellablePromise<T> {
    return this.request<T>(path, params, { ...reqInit, method: "get" })
  }

  post<T = D>(path: string, params?: P, reqInit: RequestInit = {}): CancellablePromise<T> {
    return this.request<T>(path, params, { ...reqInit, method: "post" })
  }

  put<T = D>(path: string, params?: P, reqInit: RequestInit = {}): CancellablePromise<T> {
    return this.request<T>(path, params, { ...reqInit, method: "put" })
  }

  patch<T = D>(path: string, params?: P, reqInit: RequestInit = {}): CancellablePromise<T> {
    return this.request<T>(path, params, { ...reqInit, method: "patch" })
  }

  del<T = D>(path: string, params?: P, reqInit: RequestInit = {}): CancellablePromise<T> {
    return this.request<T>(path, params, { ...reqInit, method: "delete" })
  }

  protected request<D>(path: string, params?: P, init: RequestInit = {}): CancellablePromise<D> {
