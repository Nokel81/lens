...
export const metricsApi = {
  async getMetrics<T = IMetricsQuery>(query: T, reqParams: MetricsReqParams = {}): Promise<T extends Record<string, any> ? { [K in keyof T]: Metrics } : Metrics> {
...
export interface NamedMetricsPoints {
  [name: string]: number;
}

export function getMetricLastPoints(metrics: NamedMetrics): NamedMetricsPoints {
  const result: NamedMetricsPoints = {}
